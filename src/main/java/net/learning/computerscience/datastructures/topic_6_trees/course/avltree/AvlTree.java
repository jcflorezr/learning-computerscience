...
public class AvlTree<T extends Comparable<T>> implements Tree<T> {
    ...
    private Node<T> deleteNode(Node<T> node, T data) {
        ...
        // We have to check on every deleteNode operation whether the topic_6_trees has become unbalanced or not
    ...
    private Node<T> settleDeletion(Node<T> node) {
        ...
        // we know the topic_6_trees is left heavy but it can be left-right heavy or left-left heavy
        if (balance > 1) {
        ...
        // we know the topic_6_trees is right heavy but it can be left-right heavy or right-right heavy
        if (balance < -1) {
