...
public class Solution {
    ...
    public static List<Integer> getElementsFromMaxToMin(int[] elements) {
        List<Integer> sortedElements = new ArrayList<>();
        Heap maxHeap = new Heap(elements.length);
        for (int element : elements) {
            maxHeap.insert(element);
        }
        while (!maxHeap.isEmpty()) {
            sortedElements.add(maxHeap.poll());
        }
...
class Heap {
    ...
    private void heapifyDown(int currentIndex) {
        ...
        Integer leftChild = leftChildIndex <= this.currentIndex ? heapElements[leftChildIndex] : Integer.MIN_VALUE;
        Integer rightChild = rightChildIndex <= this.currentIndex ? heapElements[rightChildIndex] : Integer.MIN_VALUE;
